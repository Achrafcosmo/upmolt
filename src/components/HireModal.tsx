              ...
              <button key={t.key} onClick={() => setTier(t.key)} className={`w-full text-left p-4 rounded-xl border transition ${tier === t.key ? 'border-um-purple bg-um-purple/5' : 'border-um-border hover:border-gray-600'}`}>
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-white font-bold text-lg">${Math.round(agent.price_usd * t.mult)}</p>
                    <p className="text-xs text-emerald-400">Save ${Math.round(agent.market_rate_usd * t.mult - agent.price_usd * t.mult)}</p>
                  </div>
                </div>
              </button>
            ))}
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea value={desc} onChange={e => setDesc(e.target.value)} placeholder="Be specific about your requirements, preferences, and any files or references..." rows={5} className="w-full bg-um-bg border border-um-border rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-um-purple resize-none" />
            </div>
            <div className="flex gap-3">
        ...
        {step === 3 && (
          <div className="space-y-4">
            <div className="bg-um-bg rounded-xl p-4 space-y-3">
              ...
              {saved > 0 && (
                <div className="flex justify-between text-sm"><span className="text-gray-400">You save</span><span className="text-emerald-400 font-bold">${saved.toLocaleString()} ({Math.round((saved / marketPrice) * 100)}%)</span></div>
              )}
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={submit} disabled={loading} className="flex-1 gradient-btn text-white py-3 rounded-xl font-medium disabled:opacity-50">{loading ? 'Submitting...' : 'Confirm & Hire'}</button>
            </div>
          </div>
        )}
      </div>

...
interface Task {
  id: string; title: string; status: string; tier: string; price_usd: number; saved_usd: number; created_at: string;
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (user) {
      fetch('/api/tasks/list').then(r => r.json()).then(d => { setTasks(d.data || []); setLoading(false) })
    }
  }, [user])
